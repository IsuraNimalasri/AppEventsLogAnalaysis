...
CREATE TABLE IF NOT EXISTS public.t_dim_browser (
 ...
 CREATE TABLE IF NOT EXISTS public.t_dim_time (
   "time_dimkey" bigint PRIMARY KEY,
   "req_ts" varchar ,
...
CREATE TABLE IF NOT EXISTS public.t_fact_view_reqs (
   "vreq_factkey" varchar PRIMARY KEY,
   "num_of_requests" int NOT NULL,
   "time_dimkey" bigint NOT NULL REFERENCES t_dim_time (time_dimkey),
   "user_dimkey" varchar REFERENCES t_dim_user (user_dimkey),
   "db_dimkey" varchar REFERENCES t_dim_dashboards (db_dimkey),
   "b_dimkey" varchar REFERENCES t_dim_browser (b_dimkey)
);
